...
unif = zeros(nrOfObjects,1);
ent = zeros(nrOfObjects,1);

for i = 1:nrOfObjects
    GCM = graycomatrix(cutouts{i}, "Offset", [5 5]);
    outp = graycoprops(GCM);
    unif(i) = outp.Energy; 
    ent(i) = entropy(cutouts{i});
end
figure();
subplot(1,2,1);
scatter(1:nrOfObjects, unif);
title("Uniformity");
subplot(1,2,2);
scatter(1:nrOfObjects, ent);
title("Entropy");